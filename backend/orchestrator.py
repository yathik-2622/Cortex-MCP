        ...
        async with stdio_client(server_params) as (read, write):
            async with ClientSession(read, write) as session:
                ...
                llm = ChatGroq(
                    ...
                    temperature=0,
                    streaming=True # Must be True for token streaming
                )

                agent_app = create_react_agent(llm, langchain_tools)
                langfuse_handler = CallbackHandler()

                # --- EXECUTION LOOP (STREAMING TOKENS) ---
                input_message = HumanMessage(content=query)

                # We use 'astream_events' with version="v2" to get granular tokens
                async for event in agent_app.astream_events(
                    ...
                    # 1. Tool Decisions (Thinking)
                    if kind == "on_tool_start":
                        yield json.dumps({
                            "type": "log",
                            "tool": event['name'],
                            "input": str(data.get('input')),
                            "message": f"Calling tool: {event['name']}..."
                        }) + "\n"

                    # 2. Tool Outputs
                    elif kind == "on_tool_end":
                        # Ignore the internal wrapper exit
                        if event['name'] != '__start__':
                            yield json.dumps({
                                "type": "log",
                                "message": f"Tool Output: {str(data.get('output'))[:200]}..."
                            }) + "\n"

                    # 3. LLM Tokens (Real-time Typing)
                    elif kind == "on_chat_model_stream":
                        chunk = data.get("chunk")
                        # Check if the chunk contains text content (ignore tool call chunks)
